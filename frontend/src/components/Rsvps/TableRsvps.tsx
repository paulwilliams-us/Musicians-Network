...
import {
  ...
  setRefetch,
  deleteItemsByIds,
} from '../../stores/rsvps/rsvpsSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureRsvpsCols';
import _ from 'lodash';
import dataFormatter from '../../helpers/dataFormatter';
import { dataGridStyles } from '../../styles';

import ListRsvps from './ListRsvps';

const perPage = 10;

const TableSampleRsvps = ({
  filterItems,
  setFilterItems,
  filters,
  showGrid,
  ...
  const {
    ...
    notify: rsvpsNotify,
    refetch,
  } = useAppSelector((state) => state.rsvps);
  ...
  useEffect(() => {
    if (rsvpsNotify.showNotification) {
      notify(rsvpsNotify.typeNotification, rsvpsNotify.textNotification);
    }
  }, [rsvpsNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/rsvps/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/rsvps/rsvps-view/?id=${id}`);
  };
  ...
  return (
    <>
      ...
      {rsvps && Array.isArray(rsvps) && !showGrid && (
        <ListRsvps
          rsvps={rsvps}
          loading={loading}
...
export default TableSampleRsvps;
