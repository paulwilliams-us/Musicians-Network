...
import {
  ...
  setRefetch,
  deleteItemsByIds,
} from '../../stores/musicians/musiciansSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureMusiciansCols';
import _ from 'lodash';
import dataFormatter from '../../helpers/dataFormatter';
import { dataGridStyles } from '../../styles';

import CardMusicians from './CardMusicians';

const perPage = 10;

const TableSampleMusicians = ({
  filterItems,
  setFilterItems,
  filters,
  showGrid,
  ...
  const {
    ...
    loading,
    count,
    notify: musiciansNotify,
    refetch,
  } = useAppSelector((state) => state.musicians);
  ...
  useEffect(() => {
    if (musiciansNotify.showNotification) {
      notify(
        musiciansNotify.typeNotification,
        musiciansNotify.textNotification,
      );
    }
  }, [musiciansNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/musicians/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/musicians/musicians-view/?id=${id}`);
  };
  ...
  return (
    <>
      ...
      {musicians && Array.isArray(musicians) && !showGrid && (
        <CardMusicians
          musicians={musicians}
          loading={loading}
...
export default TableSampleMusicians;
