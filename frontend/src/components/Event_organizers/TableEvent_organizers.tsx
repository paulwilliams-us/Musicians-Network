...
import { loadColumns } from './configureEvent_organizersCols';
import _ from 'lodash';
import dataFormatter from '../../helpers/dataFormatter';
import { dataGridStyles } from '../../styles';

import CardEvent_organizers from './CardEvent_organizers';

const perPage = 10;

const TableSampleEvent_organizers = ({
  filterItems,
  setFilterItems,
  filters,
  showGrid,
  ...
  const {
    ...
    loading,
    count,
    notify: event_organizersNotify,
    refetch,
  } = useAppSelector((state) => state.event_organizers);
  ...
  useEffect(() => {
    if (event_organizersNotify.showNotification) {
      notify(
        event_organizersNotify.typeNotification,
        event_organizersNotify.textNotification,
      );
    }
  }, [event_organizersNotify.showNotification]);
  ...
  const handleEditAction = (id: string) => {
    router.push(`/event_organizers/${id}`);
  };

  const handleViewAction = (id: string) => {
    router.push(`/event_organizers/event_organizers-view/?id=${id}`);
  };
  ...
  return (
    <>
      ...
      {event_organizers && Array.isArray(event_organizers) && !showGrid && (
        <CardEvent_organizers
...
export default TableSampleEvent_organizers;
