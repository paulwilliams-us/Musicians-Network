...
import { loadColumns } from './configureAvailabilityCols';
...
const TableSampleAvailability = ({
  ...
  useEffect(() => {
    if (availabilityNotify.showNotification) {
      notify(
        availabilityNotify.typeNotification,
        availabilityNotify.textNotification,
      );
    }
  }, [availabilityNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/availability/availability-view/?id=${id}`);
  };

  const handleCreateEventAction = ({ start, end }: SlotInfo) => {
    router.push(
      `/availability/availability-new?dateRangeStart=${start.toISOString()}&dateRangeEnd=${end.toISOString()}`,
    );
  };
    ...
    <>
      ...
      {!showGrid && (
        <BigCalendar
          ...
          start-data-key={'available_from'}
          end-data-key={'available_to'}
...
export default TableSampleAvailability;
