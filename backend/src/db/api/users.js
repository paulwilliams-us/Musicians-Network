...
module.exports = class UsersDBApi {
  ...
  static async findBy(where, options) {
    ...
    output.agents_user = await users.getAgents_user({
      transaction,
    });

    output.event_organizers_user = await users.getEvent_organizers_user({
      transaction,
    });

    output.jobs_posted_by = await users.getJobs_posted_by({
      transaction,
    });

    output.musicians_user = await users.getMusicians_user({
      transaction,
    });

    output.rsvps_user = await users.getRsvps_user({
      transaction,
    });

    output.venues_user = await users.getVenues_user({
      transaction,
    });
